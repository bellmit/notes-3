...
先使用命令进行开启：`SET GLOABL innodb_status_output_locks=1;`

然后使用命令查看当时的锁：`SHOW ENGINE INNODB STATUS;`

# 二、Server层的锁

## 1、FLUSH TABLE WITH READ LOCK

关闭所有打开的表，然后使用**全局读锁**锁定所有数据库中的所有表。此时，所有的数据更新语句（包括增删改）、数据定义语句（建表、修改表结构等）操作会被堵塞。
